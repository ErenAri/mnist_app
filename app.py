...
# Model yükle (öncelik updated_model.h5)
...
if st.button(_["predict_btn"]):
    if canvas_result.image_data is not None:
        ...
        Image.fromarray(img_resized.astype(np.uint8)).save(os.path.join(folder, filename))
...
# Yeniden eğitme butonu
st.markdown("---")
if st.button("🧠 Geri Bildirim Verileriyle Modeli Yeniden Eğit"):
    with st.spinner("Model yeniden eğitiliyor..."):
        try:
            result = subprocess.run(["python", "retrain_model.py"], capture_output=True, text=True)
            st.success("✅ Yeni model başarıyla eğitildi ve 'updated_model.h5' olarak kaydedildi.")
            st.code(result.stdout)
        except Exception as e:
            st.error("❌ Model eğitilirken bir hata oluştu.")
            st.error(str(e))
